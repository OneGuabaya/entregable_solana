...
import EditModal from "@/components/EditModal"; // Importamos el nuevo modal
...
export default function Home() {
    ...
    const [isEditOpen, setIsEditOpen] = useState(false); // Estado para editar
    const [selectedDev, setSelectedDev] = useState<any | null>(null);
    const [searchTerm, setSearchTerm] = useState("");

    useEffect(() => { setMounted(true); }, []);
    useEffect(() => {
        if (mounted && wallet) fetchDevelopers();
    ...
    const fetchDevelopers = async () => {
        ...
        try {
            ...
            setDevelopers(accounts.map((acc: any) => ({ ...acc.account, publicKey: acc.publicKey.toBase58() })));
    ...
    const deleteProfile = async () => {
        ...
        const program = new Program(idl as Idl, provider);
        try {
            const [devAccount] = web3.PublicKey.findProgramAddressSync([Buffer.from("developer"), wallet.publicKey.toBuffer()], program.programId);
            const deleteMethod = (program.methods as any).delete_developer || (program.methods as any).deleteDeveloper;
            await deleteMethod().accounts({ dev_account: devAccount, user: wallet.publicKey }).rpc({ skipPreflight: true });
            setDevelopers(prev => prev.filter(d => d.owner.toBase58() !== wallet.publicKey.toBase58()));
            setTimeout(() => fetchDevelopers(), 2000);
        } catch (err) { alert("Error al borrar."); }
    };

    const filteredDevs = useMemo(() => {
        return developers.filter((dev) => `${dev.name} ${dev.lastName} ${dev.techs}`.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    return (
        <main className="min-h-screen p-8 md:p-20 max-w-7xl mx-auto bg-transparent text-white relative z-10" suppressHydrationWarning>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {loading ? <p className="italic text-slate-500">Sincronizando con el ledger...</p> : filteredDevs.map((dev, i) => (
                    <div key={i} className="glass-card p-8 rounded-[30px] border border-white/10 bg-slate-900/40 backdrop-blur-xl border-t-indigo-500/30 flex flex-col hover:scale-[1.02] transition-transform">
                        <div className="flex justify-between mb-6">
                            ...
                            <span className="text-green-400 font-bold">${dev.hourlyRate.toString()} <small className="text-slate-500 text-[10px]">USD/H</small></span>
                        </div>
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => setSelectedDev(dev)} className="flex-1 py-3 bg-white/5 hover:bg-indigo-600 rounded-xl text-[10px] font-bold transition-all uppercase tracking-widest border border-white/10">Ver Detalle</button>

                            {dev.owner.toBase58() === wallet.publicKey.toBase58() && (
                                <>
                                    <button onClick={() => { setSelectedDev(dev); setIsEditOpen(true); }} className="p-3 bg-indigo-500/10 hover:bg-indigo-500 text-indigo-400 hover:text-white rounded-xl transition-all border border-indigo-500/20">
            ...
            {/* MODAL VER DETALLE */}
            {selectedDev && !isEditOpen && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/90 backdrop-blur-lg">
                    <div className="glass-card max-w-lg w-full p-10 border border-white/20 rounded-[40px] bg-slate-950 shadow-2xl relative">
                        <button onClick={() => setSelectedDev(null)} className="absolute top-6 right-6 text-slate-500 hover:text-white text-3xl">×</button>
                        <h2 className="text-3xl font-black text-white mb-2">{selectedDev.name} {selectedDev.lastName}</h2>
                        <p className="text-indigo-400 text-sm mb-6 uppercase tracking-widest">{selectedDev.city}, {selectedDev.country}</p>

                        <div className="space-y-4">
                            <div className="bg-white/5 p-4 rounded-xl border border-white/10">
                                <p className="text-slate-500 text-[10px] uppercase">Tech Stack</p>
                                <p className="text-white font-bold">{selectedDev.techs}</p>
                            </div>
                            <div className="bg-white/5 p-4 rounded-xl border border-white/10">
                                <p className="text-slate-500 text-[10px] uppercase">Contacto Directo</p>
                                <p className="text-white font-bold">{selectedDev.contact}</p>
                            </div>
                        </div>

                        <div className="flex gap-4 mt-8">
                            <a href={`mailto:${selectedDev.contact}`} className="flex-1 py-4 bg-indigo-600 text-white text-center rounded-2xl font-black shadow-lg shadow-indigo-500/20">CONTRATAR</a>
                            <button onClick={() => setSelectedDev(null)} className="px-6 py-4 bg-white/5 text-white rounded-2xl border border-white/10">Cerrar</button>
                        </div>
                        <p className="mt-6 text-[8px] text-slate-600 text-center font-mono">OWNER: {selectedDev.publicKey}</p>
                    </div>
                </div>
            )}

            {isModalOpen && <RegisterModal onClose={() => { setIsModalOpen(false); fetchDevelopers(); }} />}
            {isEditOpen && <EditModal dev={selectedDev} onClose={() => { setIsEditOpen(false); setSelectedDev(null); fetchDevelopers(); }} />}
        </main>
    );
}